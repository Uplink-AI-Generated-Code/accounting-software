...
The generated schema-rebuild renames `isa_parent_id` → `parent_id` (handled automatically by the temp-table-rebuild pattern's column mapping, not by a hand-written data statement) and adds the FK constraint. Applies to each real tax-year database the same on-demand way every other migration in this project does — picked up the next time each file is switched into and migrated, not all at once.
