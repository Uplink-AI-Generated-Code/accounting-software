...
export function AccountCard({ a, accounts, onSelect }) {
  return (
    <button onClick={() => onSelect(a.id)} className="text-left p-4 rounded" style={{ background: C.card, border: `1px solid ${C.line}` }}>
      ...
      {a.type === "isa-parent" ? (
        ...
        <>
          <div className="ll-mono" style={{ fontSize: 16, marginTop: 8, color: C.ink }}>{fmtUnits(a.balance || 0)} <span style={{ fontSize: 13, color: C.inkFaint }}>units</span></div>
          <div className="ll-mono" style={{ fontSize: 13, marginTop: 2, color: C.ink }}>{fmt(a.portfolioValue || 0, a.currency)} <span style={{ color: C.inkFaint }}>worth</span></div>
          <div className="ll-mono" style={{ fontSize: 12, marginTop: 1, color: C.inkFaint }}>{fmt(a.costBasis || 0, a.currency)} cost basis</div>
        </>
      ) : (
